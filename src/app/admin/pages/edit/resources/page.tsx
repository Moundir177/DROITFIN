...
export default function EditResourcesPage() {
  ...
  useEffect(() => {
    if (isClient) {
      console.log('EditResourcesPage: Language changed, refreshing content');
  ...
  const loadContent = useCallback(async () => {
    setIsLoading(true);
    try {
      console.log('EditResourcesPage: Loading resources page content');
      
      // Get the complete resources page content
      const content = await getExactPageContent('resources');
      
      if (content) {
        // Make sure we have all required sections for the resources page
        const requiredSectionIds = [
          'intro', 'legal_documents', 'guides', 'toolkits', 'videos', 'podcasts',
          'infographics', 'research_papers', 'case_studies', 'training_materials',
          'publications', 'external_resources', 'downloads'
        ];
        
        // Check if any required sections are missing
        const existingSectionIds = content.sections.map(section => section.id);
        
        console.log('EditResourcesPage: Available sections:', existingSectionIds.join(', '));
        console.log('EditResourcesPage: Required sections:', requiredSectionIds.join(', '));
        
        const missingSectionIds = requiredSectionIds.filter(id => !existingSectionIds.includes(id));
        
        if (missingSectionIds.length > 0) {
          console.log('EditResourcesPage: Missing sections:', missingSectionIds.join(', '));
        ...
        console.log(`EditResourcesPage: Content loaded with ${content.sections.length} sections`);
        setPageData(content);
      } else {
        console.error('EditResourcesPage: No content found for resources page');
      }
    } catch (error) {
      console.error('EditResourcesPage: Error loading content:', error);
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleSave = async (content: PageContent): Promise<boolean> => {
    try {
      console.log('EditResourcesPage: Saving resources page content with sections:', 
      ...
      if (success) {
        ...
        console.log('EditResourcesPage: Content saved successfully, dispatching update events');
        
        // Dispatch a custom event to notify all components that content has been updated
        try {
          window.dispatchEvent(new Event(CONTENT_UPDATED_EVENT));
          console.log(`EditResourcesPage: Dispatched ${CONTENT_UPDATED_EVENT} event`);
        } catch (error) {
        ...
        try {
          ...
          console.log(`EditResourcesPage: Dispatched storage events for page_${content.id} and editor_${content.id}`);
        } catch (error) {
      ...
      console.error('EditResourcesPage: Error saving content:', error);
      return false;
    }
  };
      ...
      <main className="flex-1 p-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <p className="text-gray-600 mb-6">
            {language === 'fr' 
              ? 'Modifiez le contenu de la page Ressources ci-dessous. Les modifications seront visibles sur le site après l\'enregistrement.'
