...
export default function EditProgramsPage() {
  ...
  useEffect(() => {
    if (isClient) {
      console.log('EditProgramsPage: Language changed, refreshing content');
  ...
  const loadContent = useCallback(async () => {
    setIsLoading(true);
    try {
      console.log('EditProgramsPage: Loading programs page content');
      
      // First make sure all sections are added to the programs page
      await updateProgramsPageWithAllSections();
      
      // Then get the complete programs page content
      const content = await getExactPageContent('programs');
      
      if (content) {
        // Make sure we have all required sections for the programs page
        const requiredSectionIds = [
          'intro', 'research', 'training', 'advocacy', 'implementation',
          'participatory', 'results_based', 'impact', 'impact_trained',
          'impact_partners', 'impact_workshops', 'partners'
        ];
        
        // Check if any required sections are missing
        const existingSectionIds = content.sections.map(section => section.id);
        
        console.log('EditProgramsPage: Available sections:', existingSectionIds.join(', '));
        console.log('EditProgramsPage: Required sections:', requiredSectionIds.join(', '));
        
        const missingSectionIds = requiredSectionIds.filter(id => !existingSectionIds.includes(id));
        
        if (missingSectionIds.length > 0) {
          console.log('EditProgramsPage: Missing sections:', missingSectionIds.join(', '));
        ...
        console.log(`EditProgramsPage: Content loaded with ${content.sections.length} sections`);
        setPageData(content);
      } else {
        console.error('EditProgramsPage: No content found for programs page');
      }
    } catch (error) {
      console.error('EditProgramsPage: Error loading content:', error);
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleSave = async (content: PageContent): Promise<boolean> => {
    try {
      console.log('EditProgramsPage: Saving programs page content with sections:', 
      ...
      if (success) {
        ...
        console.log('EditProgramsPage: Content saved successfully, dispatching update events');
        
        // Dispatch a custom event to notify all components that content has been updated
        try {
          window.dispatchEvent(new Event(CONTENT_UPDATED_EVENT));
          console.log(`EditProgramsPage: Dispatched ${CONTENT_UPDATED_EVENT} event`);
        } catch (error) {
        ...
        try {
          ...
          console.log(`EditProgramsPage: Dispatched storage events for page_${content.id} and editor_${content.id}`);
        } catch (error) {
      ...
      console.error('EditProgramsPage: Error saving content:', error);
      return false;
    }
  };
